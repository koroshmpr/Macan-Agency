...
if (have_rows('gallery')): ?>
    <ul class="slides row justify-content-center px-0 list-unstyled g-1">
        ...
        while (have_rows('gallery')): the_row();
            ...
            $thumb = wp_get_attachment_image_src( $image['ID'], '350-thumbnail' ); // 'thumbnail' can be replaced with any available image size in your theme
            $video = get_sub_field('video');
            $cover = get_sub_field('cover');
            $thumb_cover = wp_get_attachment_image_src( $cover['ID'], '350-thumbnail' ); // 'thumbnail' can be replaced with any available image size in your theme
            if ($media_type == 'Image') { ?>
                <li class="<?= $col_class; ?>">
